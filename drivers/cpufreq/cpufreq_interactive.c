		...
		const char *buf, size_t count)
{
	...
	tunables->prev_timer_rate = usecs_to_jiffies(val_round);
	...
	for_each_possible_cpu(cpu) {
		...
		if (t && t->use_sched_load) {
			...
			t->prev_timer_rate = usecs_to_jiffies(val_round);
		}
	}
					...
					struct cpufreq_policy *policy)
{
	...
	tunables->prev_timer_rate = usecs_to_jiffies(DEFAULT_TIMER_RATE);
